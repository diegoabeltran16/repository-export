# tests/test-rep-export-LINUXandMAC/test_detect_root_UNIX.py
"""
Tests unitarios para rep_export_LINUXandMAC/detect_root.py
...
def _load_detect_root():
    module_path = Path(__file__).resolve().parents[2] / "rep_export_LINUXandMAC" / "detect_root.py"
    spec = importlib.util.spec_from_file_location("detect_root_unix", str(module_path))
...
def test_env_var_invalid_prints_warning(tmp_path, monkeypatch, capsys):
    ...
    repo.mkdir()
    (repo / ".git").mkdir()
    result = find_repo_root(repo / "rep_export_LINUXandMAC" / "module.py")
    assert result == repo
...
def test_single_git_root(tmp_path):
    ...
    repo = tmp_path / "my_repo"
    inner = repo / "rep_export_LINUXandMAC"
...
def test_nested_repos_returns_outermost(tmp_path):
    ...
    inner.mkdir()
    (inner / ".git").mkdir()
    result = find_repo_root(inner / "rep_export_LINUXandMAC" / "module.py")
    assert result == outer
...
def test_no_git_returns_fallback(tmp_path, capsys):
    ...
    module_dir = tmp_path / "rep_export_LINUXandMAC"

# tests/test-rep-export-Windows/test_integration_nested.py
"""
Test de integración – Windows:
...
# Añadir rep_export_Windows al path (necesario para cli_utils_Windows,
# tag_mapper_windows y detect_root cuando se cargan vía importlib)
...
def test_exporter_reads_outermost_repo(tmp_path, monkeypatch):
    ...
    detect_mod = _load_module("_detect_root_int_w", "rep_export_Windows/detect_root.py")
    detected_root = detect_mod.find_repo_root(tool / "rep_export_Windows" / "module.py")
    ...
    exp_mod = _load_module(
        "_tiddler_exp_int_w", "rep_export_Windows/tiddler_exporter_windows.py"
    )

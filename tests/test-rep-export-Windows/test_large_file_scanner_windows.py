# tests/test-rep-export-Windows/test_large_file_scanner_windows.py
...
def _make_repo(tmp_path, files: dict[str, int]) -> Path:
    """Create tmp repo with files of given sizes (bytes)."""
    repo = tmp_path / "repo"
    repo.mkdir()
...
def test_suggest_max_below_1mb():
    # If median is below 512 KB, result should be at least 1 MB
    result = suggest_max(100 * 1024)   # 100 KB median
    assert result >= 1 * 1024 * 1024

def test_suggest_max_scales_with_median():
    median = 3 * 1024 * 1024   # 3 MB
...
def test_stats_mean_median(tmp_path):
    # Three files: 100, 300, 500 bytes → mean=300 B = 0.293 KB, median=300 B = 0.293 KB
...
# ── build_large_tiddler (via tiddler_exporter_windows) ──────────────────────
...
def _load_exporter(tmp_path, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("tiddler_exporter_w", str(module_path))
    mod = importlib.util.module_from_spec(spec)
    sys.modules["tiddler_exporter_w"] = mod
